...
//get rows query
$query = $con->prepare("SELECT name,description,valuel,endl,created_at,signature FROM tbl_infp");
							$query->execute();
...
if($rowCount > 0){
	while($row = $query->fetch(PDO::FETCH_ASSOC)){
		$post_list[] = array(     "Customer Name"=>$row["name"],"Description"=>$row["description"],"Contract Value"=>$row["valuel"], "End Date"=>$row["endl"],"Created At"=>$row["created_at"],"Signature"=>$row["signature"] );
		//$sno++;
	}
}
