...
//get rows query
$query = $con->prepare("SELECT name,description,deadline,created_at,status FROM tbl_info");
							$query->execute();
...
if($rowCount > 0){
	while($row = $query->fetch(PDO::FETCH_ASSOC)){
		$post_list[] = array(     "Task Name"=>$row["name"],"Description"=>$row["description"],"Deadline"=>$row["deadline"],"Created At"=>$row["created_at"],"Status"=>$row["status"] );
		//$sno++;
	}
}
