...
header("Content-Disposition: attachment; filename='subscriptions.xls'");
...
//get rows query
$query = $con->prepare("SELECT name,amount,renewal_date,created_at,status FROM tbl_infk");
							$query->execute();
...
if($rowCount > 0){
	while($row = $query->fetch(PDO::FETCH_ASSOC)){
		$post_list[] = array(     "Subscription Name"=>$row["name"],"Subscription Amount"=>$row["amount"],"End Date"=>$row["renewal_date"],"Subscribed Date"=>$row["created_at"],"Status"=>$row["status"] );
		//$sno++;
	}
}
